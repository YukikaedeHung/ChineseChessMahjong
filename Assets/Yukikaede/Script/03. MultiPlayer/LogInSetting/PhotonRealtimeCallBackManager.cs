...
namespace Tim.Game
{
    public class PhotonRealtimeCallBackManager : MyPhotonRealtimeCallBack
    {
        ...
        /// 內部相關的Callback均是以當前得使用者來作回應，而非房間內的其他成員
        /// 範例 Room中的聊天室窗刷新：玩家1離開房間，此時當事人本身的聊天室窗會被刷新，但留在房間內的其他人仍會擁有原本的聊天資訊
        /// </summary>
        ...
        public override void OnMasterClientSwitched(Player newMasterClient)
        {
            ...
            string s = string.Format("Change Host : {0}", newMasterClient.NickName);
            MultiPlayerSettingManagerScript.inst.TipsEven(s);
        }
        #endregion
        ...
        {        
            ...
            {
                bool bAllReady = true;
                foreach (var i in dictPlayerReadys)
                {
                    if ((bool)i.Value == false)
                    {
                        bAllReady = false;
                        break;
                    }
